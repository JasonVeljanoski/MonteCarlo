...
public class MyState implements Cloneable {
    ...
    public myState(MyState copyState) {
        ...
        this.numberOfCards = copyState.numberOfCards;
     ...
     **/
    public void newRound(Card[] remainingCards, Card topCard, Card inHand, int plrIndex, boolean[] eliminated, int topIndex, Card[][] discards, boolean[] handmaid) throws IllegalActionException {
        if (this.player != -1)
        ...
        tempDeck[numCards] = topCard;

        // Shuffles the deck, then ensures that topCard is at the top of the deck
        tempDeck = shuffle(random, tempDeck);
        tempDeck = moveToTop(tempDeck, topCard, inHand, numRemaining);

        Card[] newDeck = new Card[16];
        for (int i = topIndex; i < newDeck.length; i++) {
            newDeck[i] = tempDeck[i];
        }
        deck = newDeck;
     ...
     */
    public Card[] moveToTop(Card[] d, Card top, Card hand, int numRemaining) {
